...
/- ================================================================
   9b. 按钮按下 + 攻击怪物 Exec 证明
   ================================================================ -/

/-- Phase 1 + 按钮按下记录：走到按钮 tile 后记录按钮已按下 -/
theorem phase1_with_button_press : Exec initSym initBelief
    ([Action.right, Action.down, Action.down, Action.down, Action.down, Action.down] ++ [Action.wait])
    s1_atButton
    { initBelief with step := 7, pressedButtons := [ROOM0_BUTTON] } := by
  have h_phase1 := phase1_spawn_to_button
  -- 按钮按下 Step
  have h_press : Step s1_atButton {initBelief with step := 6} Action.wait
      s1_atButton {initBelief with step := 7, pressedButtons := [ROOM0_BUTTON]} := by
    have hpos : s1_atButton.player.isSome := by simp [s1_atButton]
    have hbutton : ROOM0_BUTTON ∈ s1_atButton.buttons := by
      simp [s1_atButton, initSym, ROOM0_BUTTON]
    have hat : s1_atButton.player.get hpos = ROOM0_BUTTON := by
      simp [s1_atButton, initSym, ROOM0_BUTTON]
    exact Step.pressButton hpos hbutton hat
  apply exec_append h_phase1
  refine Exec.cons h_press ?_
  exact Exec.nil

/-- Exec: 从 Room 0 spawn(1,1) → 走到怪物 (7,4) 相邻格 → 攻击击杀
    路径: (1,1)→...→(7,3), 然后 buttonA
    步数: right×3 + up + right×3 + down×3 + buttonA = 11 步 -/
theorem room0_attack_monster (b : BeliefState) :
    Exec (getRoomObs 0 ROOM0_SPAWN) b
      ...
      ({ (getRoomObs 0 (7,3)) with
          monsters := (getRoomObs 0 (7,3)).monsters.erase ROOM0_MONSTER })
      { b with step := b.step + 11, killedMonsters := ROOM0_MONSTER :: b.killedMonsters } := by
  let s0 := getRoomObs 0 ROOM0_SPAWN
  let s1 : SymbolicObs := {s0 with player := some (2,1), facing := Direction.right}
  let s2 : SymbolicObs := {s0 with player := some (3,1), facing := Direction.right}
  let s3 : SymbolicObs := {s0 with player := some (4,1), facing := Direction.right}
  let s4 : SymbolicObs := {s0 with player := some (4,0), facing := Direction.up}
  let s5 : SymbolicObs := {s0 with player := some (5,0), facing := Direction.right}
  let s6 : SymbolicObs := {s0 with player := some (6,0), facing := Direction.right}
  let s7 : SymbolicObs := {s0 with player := some (7,0), facing := Direction.right}
  let s8 : SymbolicObs := {s0 with player := some (7,1), facing := Direction.down}
  let s9 : SymbolicObs := {s0 with player := some (7,2), facing := Direction.down}
  let s10 : SymbolicObs := {s0 with player := some (7,3), facing := Direction.down}
  have hg0 : s0.grid = buildRoom0Grid := by simp [s0, getRoomObs]
  have hg1 : s1.grid = buildRoom0Grid := by simp [s1, s0, getRoomObs]
  have hg2 : s2.grid = buildRoom0Grid := by simp [s2, s0, getRoomObs]
  have hg3 : s3.grid = buildRoom0Grid := by simp [s3, s0, getRoomObs]
  have hg4 : s4.grid = buildRoom0Grid := by simp [s4, s0, getRoomObs]
  have hg5 : s5.grid = buildRoom0Grid := by simp [s5, s0, getRoomObs]
  have hg6 : s6.grid = buildRoom0Grid := by simp [s6, s0, getRoomObs]
  have hg7 : s7.grid = buildRoom0Grid := by simp [s7, s0, getRoomObs]
  have hg8 : s8.grid = buildRoom0Grid := by simp [s8, s0, getRoomObs]
  have hg9 : s9.grid = buildRoom0Grid := by simp [s9, s0, getRoomObs]
  have hg10 : s10.grid = buildRoom0Grid := by simp [s10, s0, getRoomObs]
  let b0 := b
  let b1 := {b0 with step := b0.step + 1}
  let b2 := {b1 with step := b1.step + 1}
  let b3 := {b2 with step := b2.step + 1}
  ...
  have h0 : Step s0 b0 Action.right s1 b1 := by
    simpa [s1, s0] using step0_right s0 b0 1 1 hg0 (by simp [s0, getRoomObs, ROOM0_SPAWN]) (by native_decide)
  have h1 : Step s1 b1 Action.right s2 b2 := by
    simpa [s2, s1, s0] using step0_right s1 b1 2 1 hg1 (by simp [s1]) (by native_decide)
  have h2 : Step s2 b2 Action.right s3 b3 := by
    simpa [s3, s2, s0] using step0_right s2 b2 3 1 hg2 (by simp [s2]) (by native_decide)
  have h3 : Step s3 b3 Action.up s4 b4 := by
    simpa [s4, s3, s0] using step0_up s3 b3 4 1 hg3 (by simp [s3])
      (by
        have hmem : (4, 0) ∈ full_pathPositions := by native_decide
        simpa [show (4, 1-1) = (4, 0) by native_decide] using hmem)
  have h4 : Step s4 b4 Action.right s5 b5 := by
    simpa [s5, s4, s0] using step0_right s4 b4 4 0 hg4 (by simp [s4]) (by native_decide)
  have h5 : Step s5 b5 Action.right s6 b6 := by
    simpa [s6, s5, s0] using step0_right s5 b5 5 0 hg5 (by simp [s5]) (by native_decide)
  have h6 : Step s6 b6 Action.right s7 b7 := by
    simpa [s7, s6, s0] using step0_right s6 b6 6 0 hg6 (by simp [s6]) (by native_decide)
  have h7 : Step s7 b7 Action.down s8 b8 := by
    simpa [s8, s7, s0] using step0_down s7 b7 7 0 hg7 (by simp [s7]) (by native_decide)
  have h8 : Step s8 b8 Action.down s9 b9 := by
    simpa [s9, s8, s0] using step0_down s8 b8 7 1 hg8 (by simp [s8]) (by native_decide)
  have h9 : Step s9 b9 Action.down s10 b10 := by
    simpa [s10, s9, s0] using step0_down s9 b9 7 2 hg9 (by simp [s9]) (by native_decide)
  -- 攻击怪物
  have hatk : Step s10 b10 Action.buttonA
      ({s10 with monsters := s10.monsters.erase ROOM0_MONSTER})
      {b10 with killedMonsters := ROOM0_MONSTER :: b10.killedMonsters, step := b10.step + 1} := by
    have hpos : s10.player.isSome := by simp [s10]
    have hmonster : ROOM0_MONSTER ∈ s10.monsters := by
      simp [s10, s0, getRoomObs, ROOM0_MONSTER]
    have hadj : adjacent (s10.player.get hpos) ROOM0_MONSTER := by
      simp [s10, ROOM0_MONSTER, adjacent, manhattan]
    exact Step.attackMonster (b := b10) hpos hmonster hadj
  -- 拼接
  ...
  refine Exec.cons hatk ?_
  -- 对齐最终状态
  have h_final_state : {s10 with monsters := s10.monsters.erase ROOM0_MONSTER} =
      ({ (getRoomObs 0 (7,3)) with monsters := (getRoomObs 0 (7,3)).monsters.erase ROOM0_MONSTER }) := by
    dsimp [s10, s0]; simp [getRoomObs]
  have h_final_belief : {b10 with killedMonsters := ROOM0_MONSTER :: b10.killedMonsters, step := b10.step + 1} =
      { b with step := b.step + 11, killedMonsters := ROOM0_MONSTER :: b.killedMonsters } := by
    simp [b10, b9, b8, b7, b6, b5, b4, b3, b2, b1, b0]
